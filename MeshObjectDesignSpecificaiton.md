...
Fortran / Keep 路径通过 C 桥获取当前 MOId：

```text
GetMeshObjectId(hDb, femId, &moId)
    → femmgMeshObjectIdManager::GetMeshObjectId()
```
...
public:
    ...
    int  GetMeshObjectId() const;   // 返回当前 MOId，不修改
    void Allocate();                // MO 创建成功后推进计数器
...
- `0` 保留为未归属
- MOId 从 `1` 开始单调递增
- `GetMeshObjectId()` 与 `Allocate()` 分离：取 Id 与消耗 Id 各司其职
...
- 位于 `iADPtr` 后
- 存放于 EHEAP common 区
- 由调用方在 `ElmAdd` 前填写，取值来自 `femmgMeshObjectIdManager::GetMeshObjectId()`
...
- 在 Open 后根据已有 `MeshObjectRecord` 初始化 `m_iMeshObjectId`
- 通过 `GetMeshObjectId()` 为 element 归属和 Fortran 路径提供**当前 MOId**
- 通过 `Allocate()` 在 MeshObject **创建成功后**推进 Id 计数器
- 为 Fortran / C 桥提供 `GetMeshObjectId` 支撑
...
Open Fem:
  ...
  m_iMeshObjectId = max(iMeshObjectId) + 1
  若没有 record，则 m_iMeshObjectId = 1
```
...
```text
GetMeshObjectId():
  返回当前 m_iMeshObjectId（只读，不修改）

Allocate():
  m_iMeshObjectId++
  （在 femmgMeshObjectManager::Create() 末尾调用）
```

一次 Keep 或一次手工建网过程中，可多次调用 `GetMeshObjectId()`，返回值相同；仅在 `Create()` 成功后才调用 `Allocate()` 推进，供下一次建网使用。
...
```c
GetMeshObjectId(hDb, femId, &moId);   // element 创建前填写 iMeshObjectId
```
...
| `femmgMeshObjectIdManager` | 调用 `InitFromRecords()`，初始化 `m_iMeshObjectId` |
...
`m_iMeshObjectId` 同样不在 mf1 中持久化；Open 时由已有 `MeshObjectRecord` 的 MOId 最大值推导。
...
3. `femmgMeshObjectIdManager` 已就绪，后续 Keep / 手工建网可调用 `GetMeshObjectId()` 获取 MOId。
...
用户在当前 FEM 中新建网格后，系统应创建对应的 MeshObject，并在 Navigator 中显示 Mesh 节点。创建分为两条路径：

| 路径 | 产品阶段 | 触发时机 |
|---|---|---|
| **几何 Keep** | 阶段一 | 用户对几何执行 Keep 且操作成功 |
| **手工建网** | 阶段二 | 用户一次手工创建节点、单元操作完成时 |

两条路径的共同点：

- 每次创建对应**一个** MeshObject
- 本次新生成的 element 归属同一个 `iMeshObjectId`
- 创建是**显式业务操作**，不在 `ElmAdd` 内自动推断

Preview 不创建 MeshObject。

#### 5.2.2 前置条件

- 当前 FEM 已打开，`femmgMeshObjectManager` 与 `femmgMeshObjectIdManager` 已就绪。
- `femdaMOFSIProxy` 已打开当前 Fem 的 `MOHEAP` / `MOTREE`。
- 对于几何 Keep：Keep 操作已成功，本次生成的 element 尚未写入归属。
- 对于手工建网：本次操作上下文已确定一组待归属的 node / element。
...
1. `m_MOIdToRecordMap` 包含新创建的 `MeshObjectRecord`。
2. 本次归属的 element 均已设置 `iMeshObjectId = moId`。
3. `m_MOIdToElementCountMap[moId]` 等于本次归属的 element 数量。
4. Navigator 中出现新 Mesh 节点，显示名称为 `szDisplayName`。
5. 用户可立即对该节点执行 Info / Rename / Delete / Show-Hide。
...
3. 位于 `iADPtr` 后
4. 由调用方在 `ElmAdd` 前填写，取值来自 `GetMeshObjectId()`
