...
| MeshObjectId 分配设计 | 定义 `femmgMeshObjectIdAllocator` 的职责与分配规则                     |
...
| `eMeshObjectType`      | MeshObject 网格类型（beam / shell / solid / rigid / constraint / unknown 等），Create 时由调用方传入并写入 MOHEAP，Rename 不改变 |
| Meshes Collector 分组  | Navigator 中 `1D/2D/3D/Others Meshes` 归属；由 `eMeshObjectType` 枚举值区间派生，不单独持久化 |
...
- `eMeshObjectType`：网格类型，Create 时由**调用方传入**并持久化；不单独持久化 Meshes Collector 分组

**Meshes Collector 分组（Navigator 用，不落盘）**

由 `eMeshObjectType` 枚举值区间派生 Navigator 归属（不定义单独的 MeshesCollector 枚举）：
...
| `101 .. 200` | 2D Meshes |
| `201 .. 300` | 3D Meshes |
| `301 .. 400`，及 `UNKNOWN(0)` | Others Meshes |
...
- **不传**显示名称；`szDisplayName` 在 `Create` 内部按 FS 规则自动生成
- `eMeshObjectType` 由**创建上下文**（Keep 链路 / 手工建网完成回调）传入，Manager 不从 element 或划分设置推断
- 混合维度或无法判定时，调用方传 `FEMMG_MO_TYPE_UNKNOWN`（归属 Others Meshes，默认名 `other_unknown_N`）
...
- UI 显示名使用 `1D Meshes`、`2D Meshes` 等；Navigator 分组由 `eMeshObjectType` 枚举值区间派生
- Navigator 构建：`QueryAllMeshObjects()` → 按 Meshes Collector 分组 → 某类无 Mesh 时隐藏对应 Meshes 节点

#### 4.3.2 节点显示

- Mesh 节点显示名称来自 `MeshObjectRecord.szDisplayName`
- Mesh 的 Type / Dimension / Collector 由 `eMeshObjectType` 及类型映射表派生（Information 展示）
...
| 规则            | 说明                                                         |
| ------------- | ---------------------------------------------------------- |
| Create 时确定    | `eMeshObjectType` 由调用方传入，在 `Create()` 成功时写入 MOHEAP          |
| Rename 不改变    | 重命名只改 `szDisplayName`，`eMeshObjectType` 不变                     |
| element 变化不改变 | Remesh、部分删 element 等不导致 Mesh 在 Meshes 节点间迁移                |
| 几何 Keep | 调用方按 Keep 上下文传入类型（如 `FEMMG_MO_TYPE_SOLID` / `SHELL` / `BEAM`） |
| 手工建网 | 调用方按手工入口传入类型（如 `FEMMG_MO_TYPE_RIGID`）；混合维度传 `FEMMG_MO_TYPE_UNKNOWN` |
| 混合 / 无法判定 | `FEMMG_MO_TYPE_UNKNOWN` → Others Meshes，默认名 `other_unknown_N` |
...
2. `femmgMeshObjectManager.Create(pComps, nCmp, eType)`（内部获取 MOId、生成默认名并调用 `Allocate()`）
...
2. 一次手工建网操作完成时，调用 `Create(nullptr, 0, eType)`
3. `Create` 内部获取 MOId、生成默认名、组装 record 并调用 `Allocate()`
...
Entity 构建 MeshObjectEntity 对象视图
    ↓
Navigator 按 eMeshObjectType 派生的 Meshes Collector 分组刷新：Fem → 1D/2D/3D/Others Meshes → Mesh
```
...
Keep 成功：
    femmgMeshObjectManager.Create(pComps, nCmp, eType)
      - 调用方传入 eType（如 FEMMG_MO_TYPE_SOLID）
...
创建 node / element 过程中：
    ...
    ↓
本次手工网格操作完成时：
    femmgMeshObjectManager.Create(nullptr, 0, eType)
      - 调用方传入 eType（如 FEMMG_MO_TYPE_RIGID；混合时 FEMMG_MO_TYPE_UNKNOWN）
      - 内部 moId = GetCurrentMeshObjectId()
...
阶段一「改」仅包含 **Rename**（重命名 Mesh 节点显示名称）。用户在 Navigator 中对 Mesh 节点执行 Rename，修改 `MeshObjectRecord.szDisplayName`；`iMeshObjectId`、`eMeshObjectType`、`pComps`、element 归属及 `m_MOIdToElementCountMap` 均不变。
...
| 规则      | 说明                                                                               |
| ------- | -------------------------------------------------------------------------------- |
| 仅改名称    | Rename 只修改 `szDisplayName`；不修改 `iMeshObjectId`、`eMeshObjectType`、`pComps`、element 归属 |
| Fem 内唯一 | 新名称在当前 Fem 内须唯一；与已有 Mesh 重名时拒绝并提示，保留原名称（FS-MO-U-R01） |
...
| 失败回滚    | 校验失败（moId 不存在、名称为空、名称重复等）则不修改 map 与 MOHEAP                                            |
