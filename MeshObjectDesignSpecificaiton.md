...
| 持久化设计 | 设计 `MOHEAP`、`MOTREE` 和变长 record 布局 |
...
- `femmgMeshObjectManager` 负责 `MeshObjectRecord` 管理与持久化
- 协调 Open/Save 与运行时恢复
...
| `femmgMoFSIProxy` | 从 `MOHEAP` / `MOTREE` 反序列化 `MOHeapRecord` |
| `femmgMeshObjectManager` | 调用 `LoadFromFSI()`，填充 `m_MOIdToRecordMap` |
...
FSI 加载 FEM 数据库（含 EHEAP、MOHEAP、MOTREE）
    ↓
对每个已打开的 Fem：
    ① femmgMeshObjectManager.LoadFromFSI()
         - 经 MOTREE 索引遍历 MOHEAP
         - 反序列化每条 MOHeapRecord → MeshObjectRecord
...
AFEM 打开与 MOId remap 不在本用例范围内，见 §4.1.2。
